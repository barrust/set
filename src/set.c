...
static int __get_index(SimpleSet *set, const char *key, key_size_tt len, uint64_t hash, uint64_t *index);
...
static int __set_contains(SimpleSet *set, const char *key, key_size_tt len, uint64_t hash);
...
int set_contains(SimpleSet *set, const char *key, key_size_tt len) {
    uint64_t index, hash = set->hash_function(key, len);
    return __get_index(set, key, len, hash, &index);
}

int set_contains_str(SimpleSet *set, const char *key) {
